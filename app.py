...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.rand(N, 1)  # Random values for X between 0 and 1
    Y = mu + np.sqrt(sigma2) * np.random.randn(N, 1)  # Random values for Y with specified mean and variance

    # TODO 2: Fit a linear regression model to X and Y
    model = LinearRegression()
    model.fit(X, Y)
    slope = model.coef_[0][0]  # Extract slope from the fitted model
    intercept = model.intercept_[0]  # Extract intercept from the fitted model
